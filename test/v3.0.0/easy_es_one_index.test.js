...
const configs = [
  {node: 'http://elastic:changeme@127.0.0.1:5300', version: '5.x'},
  {node: 'http://127.0.0.1:6800', version: '6.x'},
];

configs.forEach(test);

function test(config) {
  const client = easyES(config), index = 'one_piece';
  client.setIndex(index);

  describe('v5v6 one index (setIndex)', () => {
    ...
    before(async function() {
      ...
      if (!isExist) {
        ...
        // create
        await client.create(type, luffy, firstId);
        await client.create(type, zoro, ++firstId);
        await client.create(type, nami, ++firstId);
        await client.create(type, usopp, ++firstId);
        await client.create(type, sanji, ++firstId);
        await client.create(type, chopper, ++firstId);
        await client.create(type, robin, ++firstId);
        await client.create(type, franky, ++firstId);
        await client.create(type, brook, ++firstId);
        await client.create(type, jinbe, ++firstId);
        sleep.sleep(2);
      }
    ...
    it('search', async () => {
      ...
      // 获取前10个检索结果
      const response = await client.search(type, body, 10, 0, source);
      ...
      // 设置size为1,应该只返回一条
      const oneResult = await client.search(type, body, 1);
      ...
      const aggResult = await client.search(type, aggBody, 0);
    ...
    it('get', async () => {
      const response = await client.get(type, firstId, source);
    ...
    it('create && update && increase && get && delete', async () => {
      ...
      // create
      const createRes = await client.create(type, createDoc, insertId);
      createRes.should.equal(true);

      const getCreateRes = await client.get(type, insertId);
      ...
      const defaultIncreaseRes = await client.increase(type, insertId, 'views');
      defaultIncreaseRes.should.equal(true);

      const getDefaultIncreaseRes = await client.get(type, insertId, ['views']);
      getDefaultIncreaseRes.views.should.equal(updateView + defaultIncreaseInterval);

      // increase IncreaseInterval
      const increaseIntervalRes = await client.increase(type, insertId, 'views', increaseInterval);
      increaseIntervalRes.should.equal(true);

      const getIncreaseIntervalRes = await client.get(type, insertId, ['views']);
      getIncreaseIntervalRes.views.should.equal(updateView + defaultIncreaseInterval + increaseInterval);

      // delete
      const deleteRes = await client.delete(type, insertId);
    ...
    it('count', async () => {
      const allCount = await client.count(type);
      allCount.should.be.Number;

      const simallCount = await client.count(type, body);
    ...
    it('updateMulti && deleteMulti ', async () => {
      ...
      if (!isExist) {
        ...
        await client.create(type, doc, secondId);
        await client.create(type, thirdDoc, thirdId);
        sleep.sleep(2);
      }
      ...
      const updatedRows = await client.updateMulti(type, body, updateInfo);
      ...
      const searchRes = await client.search(type, searchBody);
      searchRes.hits[0]._source.views.should.equal(updateViews);
      searchRes.hits.length.should.be.equal(2);

      // 999的那条应该一直没有变
      const getUpdateRes = await client.get(type, thirdId);
      ...
      const deletedRows = await client.deleteMulti(type, deleteBody);

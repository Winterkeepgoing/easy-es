 ...
 * 对应elastic在7.x版本中type has been deprecated
 */
class EasyESv7 {
   ...
   */
  async search(index, body, size, from, source, options = {}) {
    const opt = {
      index: index || this.index,
      body: body,
      from: from || 0,
   ...
   */
  async increase(index, id, field, interval, lang) {
    const opt = {
      index: index || this.index,
      id: id,
      body: {
  ...
  async count(index, body) {
    ...
    const opt = {index: index || this.index};
   ...
   */
  async updateMulti(index, searchBody, updateInfo, lang) {

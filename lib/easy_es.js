...
class EasyES {
   ...
   * @param index{string}    非必填
   */
  async increase(type, id, field, interval = 1, lang, index) {
    let opt = {
      index: index || this.index,
      type: type,
      id: id,
  ...
  async create(type, body, id, index) {
    ...
    let {result, created} = await this.client.create(opt);
  ...
  async delete(type, id, index) {
    let {found, result} = await this.client.delete({
      index: index || this.index,
      type: type,
      id: id,
    });
    return result === 'deleted' && found;
  };
